...
let TokenService = class TokenService {
    ...
    async generateRefreshToken(userId) {
        ...
        if (target)
            await this.tokenRepo.remove(target);
    ...
    async validateRefreshToken(token) {
        try {
            ...
            if (getToken.isRevoked)
                throw new common_1.UnauthorizedException('리프레시 토큰이 만료되었거나 폐기되었습니다. 다시 로그인해주세요.');
            if (getToken.expiresAt < new Date())
                throw new common_1.UnauthorizedException('리프레시 토큰이 만료되었습니다. 다시 로그인해주세요.');
